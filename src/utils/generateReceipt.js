...
function buildCopy({ copyLabel, isLast, data, status, settings }) {
    ...
    const backendUrl = typeof window !== 'undefined' && window.location ? (window.location.origin.includes('5173') ? 'http://localhost:5001' : window.location.origin) : 'http://localhost:5001';
    ...
    const activeLogo = getFullUrl(settings?.logoUrl) || logoBase64;
    const sigUrl    = getFullUrl(settings?.signatureUrl || settings?.authorizedSignatureUrl);
    ...
    ">
        ...
        ">
            ...
            <div style="
                ...
                line-height: 1.25;
                margin-top: 2px;
            ">${settings?.instituteName || 'Hunar Asaan Skill Center'}</div>

            <!-- Address -->
            <div style="font-size: 7.5px; color: #6b7280; line-height: 1.5;">
                ${settings?.address || 'DHA PHASE 6 LAHORE, PAKISTAN.'}
            </div>
            <div style="font-size: 7.5px; color: #6b7280;">
                ${settings?.contact || '+92 3231218000'} &nbsp;·&nbsp; ${settings?.officialEmail || 'info@hunarasaan.edu'} &nbsp;·&nbsp; ${settings?.officialWebsite || 'hunarasaan.edu'}
            </div>
        ...
        <!-- ── Meta grid ── -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px 6px; margin-bottom: 6px;">
            <div>
        ...
        <div style="text-align:center; margin-bottom:4px;">
            ${sigUrl ? `
                <div style="display:flex; justify-content:center; align-items:center; min-height:28px; margin-bottom:2px;">
                    <img src="${sigUrl}" alt="Signature Stamp" style="max-height:32px; max-width:120px; object-fit:contain;" />
                </div>
            ` : ''}
            <div style="
