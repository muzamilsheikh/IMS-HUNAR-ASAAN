...
const drawSingleChallanCopy = (doc, x, width, label, student, amountDue, dueDate, setting, logoPath) => {
    ...
    const contactStr = `${setting?.contact || '+92 3231218000'} | Email: ${setting?.officialEmail || 'info@hunarasaan.edu'} | Web: ${setting?.officialWebsite || 'hunarasaan.edu'}`;
    ...
    if (setting && setting.signatureUrl) {
        ...
        if (fs.existsSync(sigPath)) {
            try {
                doc.image(sigPath, x + width - 85, y - 14, { fit: [70, 24] });
            } catch (e) {
    ...
    doc.moveTo(x + 10, y + 6).lineTo(x + 90, y + 6).stroke('#cbd5e1');
    doc.moveTo(x + width - 90, y + 6).lineTo(x + width - 10, y + 6).stroke('#cbd5e1');
    
    const sigLabel = setting?.signatureTitle || 'Authorized Signature';
    doc.fillColor('#a09890').fontSize(5).font('Helvetica')
       .text('Depositor Signature', x + 10, y + 10, { width: 80, align: 'center' })
       .text(sigLabel, x + width - 95, y + 10, { width: 90, align: 'center' });
};
...
const { generateFeeChallanPDF } = require('../services/pdfService');
