...
import { Calculator, UserPlus, Info, Terminal, Calendar as CalendarIcon, Hash, Upload, Phone, BadgePercent, CreditCard, AlertCircle, MapPin, CreditCard as IdCardIcon, Printer, Plus, Trash2, Mail } from 'lucide-react';
...
const RegistrationForm = ({ onSuccess, editingStudent }) => {  // ✅ Accept editingStudent prop
  ...
  useEffect(() => {
    if (editingStudent) {
      ...
      if (editingStudent.secondaryEmail && !initialContacts.some(c => c.type === 'Secondary Email' || c.value === editingStudent.secondaryEmail)) {
            ...
            {additionalContacts.map((item, index) => (
              <div key={item.id || index} className="sm:col-span-2 bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3 transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const newLabel = newType === 'Secondary Mobile' ? 'Secondary Mobile Number' :
                                         newType === 'Secondary Email' ? 'Secondary Email Address' :
                                         newType === 'Guardian Phone' ? 'Guardian / Parent Phone' :
                                         newType === 'Father Email' ? 'Father / Guardian Email' :
                                         newType === 'WhatsApp' ? 'WhatsApp Number' : 'Custom Field';
                        setAdditionalContacts(prev => prev.map((c, i) => i === index ? { ...c, type: newType, label: newLabel } : c));
                      }}
                      className="bg-white border border-slate-200 text-xs font-bold rounded-lg px-3 py-1.5 text-slate-700 focus:outline-none focus:ring-2 focus:ring-secondary"
                    >
                      <option value="Secondary Mobile">Secondary Mobile Number</option>
                      <option value="Secondary Email">Secondary Email Address</option>
                      <option value="Guardian Phone">Guardian / Parent Phone</option>
                      <option value="Father Email">Father / Guardian Email</option>
                    ...
                    {item.type === 'Custom' && (
                      <input
                        type="text"
                        placeholder="Label e.g. Office Phone"
                        value={item.label}
                        onChange={(e) => {
                          const val = e.target.value;
                          setAdditionalContacts(prev => prev.map((c, i) => i === index ? { ...c, label: val } : c));
                        }}
                        className="input-field bg-white py-1 px-3 text-xs w-44 font-bold"
                      />
                    )}
                  </div>
                  <button
                    ...
                    className="px-3 py-1 bg-rose-50 text-rose-600 hover:bg-rose-100 rounded-lg transition-colors flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider"
                  >
                ...
                <div className="relative">
                  ...
                  <input
                    type={item.type.includes('Email') ? 'email' : 'tel'}
                    className="input-field pl-12 bg-white font-bold"
                    placeholder={item.type.includes('Email') ? 'secondary@example.com' : '0300 0000000'}
                    value={item.value}
                    onChange={(e) => {
                      const val = e.target.value;
                      setAdditionalContacts(prev => prev.map((c, i) => i === index ? { ...c, value: val } : c));
                    }}
                  />
                </div>
              </div>
            ...
            <div className="sm:col-span-2">
              <button
                ...
                onClick={() => setAdditionalContacts(prev => [
                  ...prev,
                  { id: Date.now().toString(), type: 'Secondary Mobile', label: 'Secondary Mobile Number', value: '' }
                ])}
                className="w-full sm:w-auto px-5 py-2.5 bg-slate-100 hover:bg-secondary/10 text-secondary border-2 border-dashed border-slate-200 hover:border-secondary/50 rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all active:scale-95 shadow-sm"
              >
                <Plus size={16} className="text-secondary" />
                Add Secondary Email / Mobile / Extra Field
              </button>
            </div>
            {/* CNIC */}
            <div>
